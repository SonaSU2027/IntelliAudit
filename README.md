...
flowchart LR
    ...
    B --> C["Explainable AI Recommendations"]
    C --> D["Interactive Cleansing"]
    D --> E["Side-by-Side Diff"]
    E --> F["Compliance Audit Report"]
```
...
```mermaid
flowchart LR
    A["Dataset"] --> B["5D Quality Scoring Engine"]
    ...
    C --> H["Dynamic Quality Score<br/>0–100%"]
    D --> H
    E --> H
    F --> H
    G --> H

    H --> I["Letter Grade<br/>A+ / A / B / C / D"]
```
...
```mermaid
flowchart TD
    A["Original Dataset<br/>Immutable Master"] -->|Deep Copy| B["Working Dataset<br/>Active Cleaning Copy"]

    A --> C["dataProfiler.js<br/>Type Inference<br/>5-Number Summary<br/>5D Quality Scoring"]

    B --> D["missingHandler.js<br/>Imputation Algorithms<br/>Two-Pass ffill/bfill<br/>Explainable Heuristics"]

    C --> E["cleaningEngine.js & PhaseCContext<br/>Action Pipeline & Rollback Execution<br/>Real-Time Delta Metric Computation"]

    D --> E

    E --> F["auditReportGenerator.js<br/>Distribution Shift Analyzer<br/>Post-Clean Verification<br/>Synthetic Benchmarking<br/>PDF & Compliance Documentation"]
```
